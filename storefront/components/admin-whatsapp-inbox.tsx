...
function friendlyWhatsAppError(value: unknown) {
  ...
  if (message === "API access blocked." || message.toLowerCase().includes("24-hour whatsapp")) {
...
export function AdminWhatsAppInbox() {
  ...
  async function send(event: FormEvent) {
    ...
    if (!hasOpenServiceWindow(messages)) {
      setError("The 24-hour WhatsApp customer-service window is closed. Use an approved Meta template (Order Confirmation or Order Update) for this customer.");
      return;
    }
    setSending(true);
    try {
  ...
  async function sendOrderUpdateTemplate(orderId: string) {
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not send order update");
    }
  }
            ...
            <>
              ...
              <form className={styles.composer} onSubmit={send}>
                <textarea
                  ...
                  rows={2}
                  placeholder={serviceWindowOpen ? "Type a reply…" : "Free-form reply unavailable outside the 24-hour window"}
                  value={draft}
                  onChange={(e) => setDraft(e.target.value)}
                  aria-label="Message"
                  disabled={sending || !serviceWindowOpen}
                />
                <button
                  type="submit"
                  disabled={sending || !draft.trim() || !serviceWindowOpen}
                  className={styles.sendBtn}
                  aria-label="Send via Meta API"
                  title={serviceWindowOpen ? "Send via official Meta WhatsApp Cloud API" : "Use an approved Meta template while the 24-hour window is closed"}
                >
                  <Send size={16} />
                </button>
              </form>
              <p className={styles.windowNote}>
                {serviceWindowOpen ? (
                  ...
                  <>💡 <strong>Meta messaging:</strong> The free-form reply window is closed. Use Order Confirmation or Order Update above.</>
                )}
              </p>
            </>
          )}
