...
export function ProductPurchase({
  ...
  return <div className="purchase-panel">
    ...
    <aside className="featured-prebook-offer">
      ...
      <p><Sparkles size={18} aria-hidden="true" /> {introPriceText}</p>
    </aside>
    <p className="coupon-offer">{referralOfferText}</p>
    <div className="pdp-khand-benefits">{benefits.map(({ title, Icon }) => <article key={title}><Icon aria-hidden="true" /><h3>{title}</h3></article>)}</div>
    <p className="pdp-ingredients"><strong>Ingredients:</strong> {product.ingredients}</p>
    <div className="purchase-block">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: "0.4rem", marginBottom: "0.4rem" }}>
        ...
        <span style={{ fontSize: "0.74rem", color: "#8a6616", fontWeight: 600, letterSpacing: "0.02em" }}>{introPriceText}</span>
      </div>
      <div className="variant-row pdp-sizes">{product.variants.map((item: ProductVariant) => <button key={item.id} type="button" aria-pressed={variantId === item.id} className={variantId === item.id ? "active" : ""} onClick={() => { setVariant(item.id); setShippingMessage(""); }}><strong>{item.label}</strong><small>{formatPrice(item.pricePaise)}</small></button>)}</div>
    </div>
    ...
    <div className="pdp-sticky-buy"><span>{product.name}<small>{variant.label} · {formatPrice(variant.pricePaise)} · <span style={{ color: "#d8b456" }}>{introPriceText}</span></small></span><button type="button" className="button button-dark" disabled={!readyForSale} onClick={buyNow}>Buy now</button></div>
  </div>;
}
