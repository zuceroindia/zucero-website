...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const isMishri = slug === "dhage-wali-mishri";
  const title = isKhand
    ? "Single Origin Desi Khand | Traditional Unrefined Cane Sugar"
    : isMishri
      ? "Original Khand Dhaga Mishri | Dhage Wali Mishri"
      : product.name;
  ...
  return {
    ...
    keywords: isKhand
      ...
      : isMishri
        ...
        : [product.name, "Zucero", "The Good Sugar"],
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="store-page pdp-page">
    ...
    <section className="pdp-difference"><p className="eyebrow">The Good Facts — {isKhand ? "Desi Khand" : isMishri ? "Mishri" : product.name}</p><h2>Good begins<br /><em>with how it’s made.</em></h2><div>{goodFacts.map(([title, copy]) => <article key={title}><Leaf /><h3>{title}</h3><p>{copy}</p></article>)}</div></section>
    <section className="transparency-section"><p className="eyebrow">Transparency</p><h2>Nothing hidden<br /><em>behind what you choose.</em></h2><p>Clear information. Specific claims. Nothing overstated.</p><p>Check your pack for the complete ingredients, manufacturing, nutrition and best-before details.</p>{(isKhand || isMishri) && <p><strong>Contains milk.</strong> These products are prepared with desi cow milk and desi cow ghee.</p>}</section>
    <section id="product-information" className="pdp-information"><div><p className="eyebrow">Every detail matters</p><h2>Get to know<br />{product.name}.</h2></div><div>
      <details open><summary>Description <span>+</span></summary><p>{product.description} {isKhand ? "Use it where you enjoy a rounded sweetness in your everyday cooking." : isMishri ? "Enjoy its crystalline texture and allow time for it to dissolve in warm drinks." : ""}</p><p>Refer to the final pack label for the complete ingredients, nutrition, usage and best-before information for this product.</p></details>
      <details><summary>Character &amp; uses <span>+</span></summary><p>{isKhand ? "A versatile choice for tea, coffee, kheer, halwa and home baking. Start with a small amount and adjust to taste." : isMishri ? "A traditional format for warm beverages and sweet preparations. Larger crystals take longer to dissolve than fine sugar." : product.description}</p></details>
      <details><summary>How to use <span>+</span></summary><p>{isKhand ? "Measure with a clean, dry spoon. Stir into hot drinks or incorporate into your recipe, adjusting the amount to your taste." : isMishri ? "Separate the sugar crystals from any thread before using them. Dissolve in a warm drink or use in a recipe. Do not consume the thread." : "Follow the usage guidance printed on the final product pack."}</p></details>
