...
function metaWhatsAppErrorMessage(result: MetaWhatsAppResponse): string {
  const error = result.error as { message?: string; code?: number } | undefined;
  if (error?.code === 200 && /api access blocked/i.test(error.message || "")) {
    return "Meta Cloud API authorization is blocked. Refresh the WhatsApp System User access token and confirm it has whatsapp_business_messaging plus access to the Zucero WhatsApp Business Account.";
  }
  return error?.message || JSON.stringify(result);
}
...
/**
 * Sends the approved Meta utility template `order_update_v1`.
 * The approved template currently uses the Zucero order number as body variable {{1}}.
 * This can be used outside the 24-hour customer-service window.
 */
export async function sendOutboundWhatsAppOrderUpdate(orderId: string): Promise<OutboundWhatsAppResult> {
  ...
  const templateName = process.env.WHATSAPP_ORDER_UPDATE_TEMPLATE_NAME?.trim() || "order_update_v1";
  const languageCode =
    process.env.WHATSAPP_ORDER_UPDATE_TEMPLATE_LANG?.trim() ||
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        template: {
          ...
          components: [
            {
              type: "body",
              parameters: [{ type: "text", text: String(order.order_number) }],
            },
          ],
        },
      }),
    ...
    if (!response.ok) {
      ...
      try {
        await db.from("payment_events").insert({
          provider: "whatsapp",
          provider_event_id: `order-update:failed:${order.id}:${Date.now()}`,
          event_type: "whatsapp.order_update_send_error",
          payload: { error: result, recipient, order_id: order.id, template: templateName },
    ...
    if (messageId) {
      await recordOutboundWhatsAppMessage({
        recipient,
        customerName: address.fullName?.trim() || undefined,
        bodyText: `Order update sent for Zucero order ${order.order_number}.`,
    ...
    try {
      await db.from("payment_events").insert({
        provider: "whatsapp",
        provider_event_id: `order-update:sent:${order.id}:${messageId || Date.now()}`,
        event_type: "whatsapp.order_update_sent",
        payload: { messageId, recipient, order_id: order.id, template: templateName },
        processed_at: new Date().toISOString(),
      });
    } catch {}
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "WhatsApp order update dispatch exception",
    };
  }
}
