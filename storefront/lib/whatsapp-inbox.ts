...
async function hasOpenCustomerServiceWindow(conversationId: string): Promise<boolean> {
  ...
  if (error || !data?.sent_at) return false;
  const lastInbound = new Date(data.sent_at).getTime();
  if (!Number.isFinite(lastInbound)) return false;
  return Date.now() - lastInbound < CUSTOMER_SERVICE_WINDOW_MS;
}

const SERVICE_WINDOW_CLOSED_MESSAGE =
  "The 24-hour WhatsApp customer-service window is closed for this customer. Use an approved Meta template such as Order Confirmation or Order Update to contact them.";

const META_AUTH_BLOCKED_MESSAGE =
  "Meta Cloud API authorization is blocked. Refresh the WhatsApp System User access token and confirm it has whatsapp_business_messaging plus access to the Zucero WhatsApp Business Account.";
...
export async function sendWhatsAppTextReply(conversationId: string, body: string) {
  ...
  if (!(await hasOpenCustomerServiceWindow(conversationId))) {
    throw new Error(SERVICE_WINDOW_CLOSED_MESSAGE);
  }
  ...
  if (!response.ok) {
    if (result.error?.code === 200 && /api access blocked/i.test(result.error?.message || "")) {
      throw new Error(META_AUTH_BLOCKED_MESSAGE);
    }
...
export async function startWhatsAppConversation(input: {
  ...
  if (!(await hasOpenCustomerServiceWindow(conversation.id))) {
    throw new Error(SERVICE_WINDOW_CLOSED_MESSAGE);
  }
